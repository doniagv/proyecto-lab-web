...
const Tracks = ({ track }) => {
  return (
    <div key={track.id} className="m-2 text-center max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-900 dark:border-gray-700">
      <a href={track.external_urls.spotify ? track.external_urls.spotify : ""}>
        {track.album.images.length ? <img className="rounded-t-lg" src={track.album.images[0].url ? track.album.images[0].url : ""} alt="" /> : <div>No Image</div>}
      </a>
      <div className="p-5">
        ...
        <a
          target="_blank"
          rel="noreferrer"
          href={track.external_urls.spotify ? track.external_urls.spotify : ""}
